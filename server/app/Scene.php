...
class Scene extends BaseClass
{
	...
	public function listScenes($user, $movie) {
		...
		$ck = $this->queryAll('SELECT sc_id, sc_title, MAX(sl_user) AS sl_user FROM (SELECT t1.`sc_id`, t1.`sc_movie`, (SELECT t2.`sc_title` FROM `' . $this->conf['databasePrefix'] . 'scenes` t2 WHERE t2.`sc_movie`=:t2mv AND t2.`sc_id`=t1.`sc_id` ORDER BY t2.`sc_uid` DESC LIMIT 1) as sc_title FROM `' . $this->conf['databasePrefix'] . 'scenes` t1 WHERE t1.`sc_movie`=:t1mv ORDER BY t1.`sc_title` ASC) tbs LEFT JOIN `' . $this->conf['databasePrefix'] . 'scenelock` lck ON (tbs.`sc_id`=lck.`sl_scene` AND tbs.`sc_movie`=lck.`sl_movie`) GROUP BY tbs.`sc_id`, lck.`sl_id` ORDER BY tbs.`sc_title` ASC', [
			':t2mv' => $movie, 
			':t1mv' => $movie, 
		], 'SELECT * FROM (SELECT t1.`sc_id`, t1.`sc_movie`, (SELECT t2.`sc_title` FROM `' . $this->conf['databasePrefix'] . 'scenes` t2 WHERE t2.`sc_movie` = :t2mv AND t2.`sc_id` = t1.`sc_id` ORDER BY t2.`sc_uid` DESC LIMIT 1) AS sc_title FROM `' . $this->conf['databasePrefix'] . 'scenes` t1 WHERE t1.`sc_movie` = :t1mv ORDER BY t1.`sc_title` ASC) tbs LEFT JOIN `' . $this->conf['databasePrefix'] . 'scenelock` lck ON tbs.`sc_id` = lck.`sl_scene` AND tbs.`sc_movie` = lck.`sl_movie` GROUP BY tbs.`sc_id`, lck.`sl_id` ORDER BY tbs.`sc_title` ASC');
		foreach ($ck as $v) $ret[] = [
				...
				if (!$ok) {
					...
					foreach ($scene['keyframes'] as $kf) {
						...
						foreach ($kf as $kins => $ins) {
							$this->execute('INSERT INTO `' . $this->conf['databasePrefix'] . 'instances` (`in_keyframe`, `in_name`, `in_collection`, `in_asset`, `in_action`, `in_play`, `in_actionover`, `in_timedac`, `in_focus`, `in_alt`) VALUES (:keyframe, :name, :collection, :asset, :action, :play, :actionover, :timedac, :focus, :alt)', [
								':keyframe' => $kid, 
								':name' => $kins, 
								...
								':focus' => $ins['focus'] ? '1' : '0', 
								':alt' => isset($ins['alttext']) ? substr($ins['alttext'], 0, 512) : '', 
							]);
							$iid = $this->insertID();
