...
class WindowContrSound extends PopupWindow {

    // current list
    private var _list:Map<String, SoundContraption> = [ ];
    ...
    **/
    public function new(ac:Dynamic) {
        // creating window
        super(ac, Global.ln.get('window-contrsound-title'), 1000, InterfaceFactory.pickValue(525, 540), false, true, true);
    }
    ...
    override public function startInterface(evt:Event = null):Void {
        ...
        this.addForm(Global.ln.get('window-kfmanage-title'), this.ui.createColumnHolder('columns', 
            this.ui.forge('leftcol', [
                { tp: 'Label', id: 'registered', tx: Global.ln.get('window-contrsound-registered'), vr: '' }, 
                { tp: 'List', id: 'registered', vl: [ ], sl: null, ht: 200 }, 
                { tp: 'Button', id: 'load', tx: Global.ln.get('window-contrsound-load'), ac: loadContr }, 
                { tp: 'Button', id: 'remove', tx: Global.ln.get('window-contrsound-remove'), ac: removeContr },
                { tp: 'Spacer', id: 'add', ht: 15, ln: true }, 
                { tp: 'Label', id: 'name', tx: Global.ln.get('window-contrsound-name'), vr: 'detail' }, 
                { tp: 'TInput', id: 'name', tx: '', vr: '' }, 
                { tp: 'Button', id: 'add', tx: Global.ln.get('window-contrsound-add'), ac: addContr },
            ]), 
            this.ui.forge('rightcol', [
                { tp: 'Label', id: 'properties', tx: Global.ln.get('window-contrsound-properties'), vr: '' }, 
                { tp: 'Label', id: 'audio', tx: Global.ln.get('window-contrsound-file'), vr: 'detail' },
                { tp: 'Custom', cont: this.ui.hcontainers['audio'] }, 
            ]),
            this.ui.forge('bottom', [
                { tp: 'Button', id: 'save', tx: Global.ln.get('window-contrsound-save'), ac: saveContr },
            ])
            , 420));
    ...
    **/
    override public function acStart():Void {
        ...
        for (contk in GlobalPlayer.contraptions.sounds) {
    ...
    **/
    ...
    private function saveContr(evt:Event):Void {
        for (cont in GlobalPlayer.contraptions.sounds.keys()) {
            GlobalPlayer.contraptions.sounds[cont].kill();
            GlobalPlayer.contraptions.sounds.remove(cont);
        }
        for (cont in this._list.keys()) {
            GlobalPlayer.contraptions.sounds[cont] = this._list[cont].clone();
        }
        this.clear();
    ...
    private function addContr(evt:Event):Void {
        if (this.ui.inputs['name'].text.length > 3) {
            if (this.ui.inputs['audio'].text != '') {
                ...
                Global.showPopup(Global.ln.get('window-contrsound-title'), Global.ln.get('window-contrsound-noaudio'), 320, 150, Global.ln.get('default-ok'));
            }           
        } else {
            Global.showPopup(Global.ln.get('window-contrsound-title'), Global.ln.get('window-contrsound-noname'), 320, 150, Global.ln.get('default-ok'));
        }
    }
    ...
    private function onSaveReturn(ok:Bool, ld:DataLoader):Void {
        ...
        if (ok) {
            if (ld.map['e'] == 1) {
                Global.showPopup(Global.ln.get('window-contrsound-title'), Global.ln.get('window-contrsound-ersave1'), 320, 150, Global.ln.get('default-ok'));
            } else if (ld.map['e'] == 2) {
                Global.showPopup(Global.ln.get('window-contrsound-title'), Global.ln.get('window-contrsound-ersave'), 320, 150, Global.ln.get('default-ok'));
            } else {
                Global.showMsg(Global.ln.get('window-contrsound-oksave'));
