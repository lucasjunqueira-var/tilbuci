...
class WindowExchangeCordova extends PopupWindow {
    ...
    **/
    public function new(ac:Dynamic) {
        // creating window
        super(ac, Global.ln.get('window-exchcord-title'), 1100, InterfaceFactory.pickValue(410, 500), false, true, true);
    }
    ...
    **/
    override public function startInterface(evt:Event = null):Void {
        ...
        this.addForm(Global.ln.get('window-exchcord-title'), this.ui.forge('desk', [
            ...
            { tp: 'Select', id: 'fullscr', vl: [
                { text: Global.ln.get('window-exchcord-fullscr-normal'), value: 'normal' }, 
                { text: Global.ln.get('window-exchcord-fullscr-full'), value: 'full' }, 
                { text: Global.ln.get('window-exchcord-fullscr-kiosk'), value: 'kiosk' }, 
            ], sl: 'complete' }, 
            { tp: 'Spacer', id: 'export', ht: 10, ln: false }, 
            { tp: 'Button', id: 'export', tx: Global.ln.get('window-exchcord-button'), ac: this.onExport }, 
            { tp: 'Spacer', id: 'nw', ht: 25, ln: true }, 
    ...
    **/
    private function onCord(evt:TriggerEvent):Void {
        var req:URLRequest = new URLRequest('https://capacitorjs.com/');
        req.method = 'GET';
        Lib.getURL(req);
    }
    ...
    **/
    private function onExport(evt:TriggerEvent):Void {
        if (this.ui.inputs['appid'].text == '') {
            this.ui.createWarning(Global.ln.get('window-exchcord-title'), Global.ln.get('window-exchcord-noappid'), 300, 180, this.stage);
        } else if (this.ui.inputs['icon'].text == '') {
            this.ui.createWarning(Global.ln.get('window-exchcord-title'), Global.ln.get('window-exchcord-noappicon'), 300, 180, this.stage);
        } else {
            Global.ws.send('Movie/ExportCordova', [
                'movie' => GlobalPlayer.movie.mvId, 
                'mode' => 'complete', // this.ui.selects['mode'].selectedItem.value, 
                ...
                'fullscr' => this.ui.selects['fullscr'].selectedItem.value, 
